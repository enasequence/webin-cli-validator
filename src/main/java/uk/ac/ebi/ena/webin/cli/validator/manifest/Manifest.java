...
public abstract class Manifest<FileType extends Enum<FileType>> {
  ...
  public List<SubmissionFile<FileType>> files(FileType fileType) {
    return files.get().stream()
        .filter(file -> file.isFileType(fileType))
  ...
  public String getWebinRestUri() {
    return webinRestUri;
  }

  public void setWebinRestUri(String webinRestUri) {
    this.webinRestUri = webinRestUri;
  }

  public String getBiosamplesUri() {
    return biosamplesUri;
  }
