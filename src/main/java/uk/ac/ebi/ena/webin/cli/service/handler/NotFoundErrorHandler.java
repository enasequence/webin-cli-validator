...
public class NotFoundErrorHandler implements ResponseErrorHandler {
    ...
    public void handleError(ClientHttpResponse httpResponse) throws IOException {
        switch (httpResponse.getStatusCode()) {
            case UNAUTHORIZED:
            case FORBIDDEN:
                throw ServiceiException.userError(ServiceMessage.CLI_AUTHENTICATION_ERROR.text());
            case NOT_FOUND:
                throw ServiceiException.validationError(validationError);
            default:
                throw ServiceiException.systemError(systemError);
        }
    }
}